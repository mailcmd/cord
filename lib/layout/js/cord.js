...
// const identifier_re = /(?<![@#$_\\p{ID_Continue}\\p{ID_Start}])(?!(?:await|break|case|catch|class|const|continue|debugger|default|delete|do|else|enum|export|extends|false|finally|for|function|if|import|in|instanceof|new|null|return|super|switch|this|throw|true|try|typeof|var|void|while|with|yield|implements|interface|let|package|private|protected|public|static|arguments|eval|globalThis|Infinity|NaN|undefined)(?![$_\\p{ID_Continue}]))[$_\\p{ID_Start}][$_\\u200C\\u200D\\p{ID_Continue}]*/gu;
...
const CORD = function() {
    ...
    const annotate = function(html) {
        const replaces = html
            .matchAll(/\$\{(.+?)\}/sg)
            .toArray()
            .map(([c, e]) => [ c, e.match(identifier_re) ]);

        html = replaces
            .reduce(
                (acc, [c, f]) => {
                    f = (new Set(f)).values().toArray().map( s => '|'+s+'|').toString();
                    if ( (/^\$\{is_table=true/).test(c) ) {
                        const a = `f='${f}' e='${btoa(c)}'`;
                        return acc.replace(c, `<tbody cord-field ${a}>`+c+'</tbody>');
                    } else {
                        const a = `f='${f}' e='${btoa(c)}'`;
                        return acc.replace(c, `<cord-field ${a}>`+c+'</cord-field>');
                    }
                }, html
            );
...
//        console.log(html);
    ...
    const expand_for = function(html) {
        ...
        const replaces = matchs
              .map( ([_, r, rows, body]) => {
                  ...
                  const is_table = body.trim().slice(0,3).toLowerCase() == '<tr';
                  body = body
                      .split('\n')
                      .map(l => `'${l}'`)
                      .join('+\n')
                  ...
                  return '${'+(is_table?'is_table=true,':'')+'('+rows+'||[]).map( __'+r+'__ => '+body+').join(\'\')}';
              });
    ...
    const bootstrap = function() {
        ...
        for (let elem of container_elems) {
            ...
            if (template) {                
                elem.cordContent = parse(template.innerHTML);
            } else {
                elem.cordContent = parse(elem.innerHTML);
            }
            elem.cordAttrs = {};
    ...
    const render_container_field = function(container_ref, field_name) {
        const elems_container =
              document.querySelectorAll(`
                *[cord-id="${container_ref}"] cord-field[f*="|${field_name}|"],
                *[cord-id="${container_ref}"] tbody[cord-field][f*="|${field_name}|"]
              `);
        console.log(elems_container)
        for (let elem of elems_container) {
            const exp = atob(elem.getAttribute('e'));
            elem.innerHTML = eval(exp, DATAS[container_ref]);
        }
    };
    ...
    this.update = function(container_ref, datas, value) {
        let fields = [], render_all = (datas == undefined);
        ...
        for (let field of fields) {
            ...
            render_container_field(container_ref, field);
        }
        
        // create globals if required
        if (this.config.createGlobals && !window[container_ref])
            window[container_ref] = this.$[container_ref];
        
        // render container
        if (render_all) render_container(container_ref);
    }
    ...
    */
    ...
    this.init = function(config = {}) {
        ...
        for (let container_ref in this.config.initials) {
            for (let field in this.config.initials[container_ref]) {
                this.$[container_ref][field] = this.config.initials[container_ref][field];
            }
        }
        
        // Renders complete all container al least once
        for (let container_ref in DATAS) {
            this.update(container_ref);
        }
...
/*
  ...
  - [ ] Create protocol for communication.
    ...
    + [ ] Define format for direct server update of cord_containers 
