...
const CORD = function() {
    ...
    const get_identifiers = function(str) {
        ...
        const local_handler = {
            get(target, prop) {
                ...
                return new Proxy({}, local_handler);
            },
            has(target, prop) {
                return true;
            }
        };
        const sandbox =  new Proxy({}, local_handler);
    ...
    const render_foreachs = function(cord_id, tpls, obj) {
        ...
        tpls.forEach( tpl => {
            ...
            arr.forEach( (r, i) => {
                ...
                const html = cord_eval(cloned_tpl.innerHTML, {
                    ...{$: DATAS},
                    ...DATAS[cord_id],
                    ...row
                });
    ...
    const render_ifs = function(cord_id, tpls) {
        ...
        tpls.forEach( tpl => {
            ...
            // if (eval(cord_eval(if_exp, env)) == false) return;
    ...
    const render_container = function(cord_id, field) {
        ...
        for (let attr in attrs) {
            if (field && !attrs[attr].match(real_field)) continue;
            elem.setAttribute(
                attr,
                cord_eval(attrs[attr], env)
            );
        }
        ...
        fields.forEach( field => {
            ...
            if (window.cordGlobalForeachs[cord_id] && window.cordGlobalForeachs[cord_id][field]) {
                window.cordGlobalForeachs[cord_id][field].forEach( tpl => tpls.add(tpl) );
            }
        });
        ...
        fields.forEach( field => {
            ...
            if (window.cordGlobalIfs[cord_id] && window.cordGlobalIfs[cord_id][field]) {
                window.cordGlobalIfs[cord_id][field].forEach( tpl => tpls.add(tpl) );
            }
        });
...
/* Temporal DOC
   ...
   Every container, after 'process_containers()' (bootstrap call this), has the following
   properties:
